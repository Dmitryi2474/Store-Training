...
const Counter = ({item}) => {
  ...
  const onClickMinus = () => {
    if (item.currentCount === 1) {
      setCart(prev => {
        return {
          cartItems: [...prev.cartItems.filter(current => current.id !== item.id)],
          total: cart.total - item.specification.price
        }
      })
    } else {
  ...
  return (
      <div className={classes.counter}>
        <button onClick={onClickMinus} className={classes.counterBtn}>
        </button>
        <h1 className={classes.number}>{item.currentCount}</h1>
        <button onClick={onClickPlus} className={classes.counterBtn}>
        </button>
      </div>
